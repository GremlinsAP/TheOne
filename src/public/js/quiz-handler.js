...
const startQuiz = (mode) =>
  postQuizData({ startQuiz: true, gamemode: mode }, () => reload(true));
...
const handleGamemode = (data) => {
  ...
  $(quizMain)
    .find(".start-quiz-ten")
    .on("click", () => startQuiz("ten"));
  $(quizMain)
    .find(".start-quiz-suddendeath")
    .on("click", () => startQuiz("suddendeath"));
};
...
const handleActive = (data) => {
  ...
  quizMain
    .find("cite")
    .find("h1")
    .text(`${data.questionIndex + 1}. ${question.Dialog}`);
  ...
  question.possibleCharacters.forEach((character) =>
    characterOptions.push(
      $(`<div class="quiz-character-option"><h2>${character.name}</h2></div>`)
    )
  );
  question.possibleMovies.forEach((movie) =>
    movieOptions.push(
      $(`<div class="quiz-movie-option"><h2>${movie.name}</h2></div>`)
    )
  );
  ...
  for (let ce = 0; ce < characterElements.length; ce++) {
    ...
    characterElement.onclick = () =>
      handleOptionSelection(
        ...
        characterElements,
        "character",
        question.possibleCharacters[ce]._id,
        submitButton[0]
      );
    movieElement.onclick = () =>
      handleOptionSelection(
        ...
        movieElements,
        "movie",
        question.possibleMovies[ce]._id,
        submitButton[0]
      );
  }
  ...
  quizFooter
    .find("h2")
    .text(
      `${data.questionIndex + 1} of ${
        data.quizType == 0 ? "Infinite" : data.questionIndexMax
      } Questions`
    );
...
const handleReview = (data) => {
  ...
  question.possibleCharacters.forEach((character) => {
    let optionEl = $(
      `<div class="flex evenspace quiz-character-option"><h2>${character.name}</h2></div>`
    );
    let correct = question.correctAnswer[0] == character._id;
    let selected = question.userAnswer[0] == character._id;
    optionEl.html(
      optionEl.html() + ` <h2>${correct && selected ? "+0.5" : ""}</h2>`
    );
    characterOptions.push(
      optionEl.addClass(
        correct ? "quiz-correct-option" : selected ? "quiz-wrong-option" : ""
      )
    );
  });
  ...
  question.possibleMovies.forEach((movie) => {
    let optionEl = $(
      `<div class="flex evenspace quiz-movie-option"><h2>${movie.name}</h2></div>`
    );
  ...
  quizFooter
    .find(".quiz-total-score")
    .text(
      `Total Score: ${reviewData.score} / ${
        data.questionIndexMax
      } (${Math.floor((reviewData.score / data.questionIndexMax) * 100)}%)`
    );
};
...
const checkAndSetNextQuestionButtonState = (submitButton) => {
  if (userAnswers.movie && userAnswers.character)
...
const getRates = async (quoteId) => {
  await fetch(`/rate/${quoteId}`, {
    ...
    .then((data) => data.json())
    .then((rateData) => (userRate = rateData));
};
